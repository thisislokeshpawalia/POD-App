...
def build_pdf(filename=r"c:\Users\me\StudioProjects\pody\PODAPP_FRS.pdf"):
    ...
    story.append(Paragraph("28. TECHNICAL EDGE CASES & ARCHITECTURAL CLARIFICATIONS", h1_style))
    story.append(Paragraph("<b>1. Partner Profile Onboarding (Postal & Address Resilience):</b> The Indian Postal API lookup implements an 8-second timeout. If the request fails, times out, or returns no records, the system displays an inline warning ('Unable to fetch location automatically') and activates a manual unlock toggle (edit-pencil icon on State field). Tapping this icon switches City & State to editable fields so operatives are never blocked. Address validation enforces a 5-character minimum and permits standard structural delimiters (e.g. '#', '/', '-', ',') to support Indian postal formats. Email validation blocks all whitespace characters and accepts all standard 2+ character top-level domains.", bullet_style))
    story.append(Paragraph("<b>2. Authentication & Rate-Limiting:</b> The mobile authentication gateway enforces a threshold of 5 consecutive failed OTP attempts on POST /auth/verify-otp before a phone number is subjected to a 15-minute temporary lockout. Whenever an API endpoint returns HTTP 401 (Unauthorized) or a JWT token expires, the client immediately throws an UnauthenticatedException, purges session credentials from flutter_secure_storage, resets AuthProvider status to unauthenticated, and navigates to the Login screen.", bullet_style))
    story.append(Paragraph("<b>3. Beneficiary Validation & Resource Authorization:</b> On-demand farmer creation requires Farmer Name (2-50 chars, alphabetic & spaces), 10-digit mobile number, Village, District, 6-digit PIN code, GPS coordinates, assigned items, and reference photo. If an operative tampers with an API payload to request a beneficiary assigned to another partner, the backend enforces scoped tenancy (delivery_partner_id filter) and returns HTTP 404 Not Found (Existence Masking) rather than 403, preventing resource enumeration.", bullet_style))
    story.append(Paragraph("<b>4. Multi-Stage Handover Protocol (Biometrics, Video & Geofence):</b> If an SMS OTP fails to reach a beneficiary in remote terrain, the pre-registered OTP in the beneficiary record can be verified via supervisory clearance. For Gate 2 facial verification, Google ML Kit extracts landmark geometry and MobileFaceNet computes Euclidean distance across L2-normalized embeddings, requiring a strict distance < 0.90 to pass. If multiple faces appear in the camera frame, the algorithm isolates the largest face by bounding box area. Gate 4 video is restricted to a 30-second live MP4 capture streamed directly to Cloudinary. For Gate 5 geofencing, LocationAccuracy.high is requested; the 600m perimeter buffer is engineered specifically to absorb rural GPS drift under dense canopy.", bullet_style))
    story.append(Paragraph("<b>5. Invoicing & PDF Synchronization:</b> Invoice PDFs are compiled locally on device storage via InvoiceService before network transmission to Cloudinary. If a network interruption occurs before receiving the Cloudinary CDN receipt URL, the app retains the local file path and delivery log data, allowing the operative to re-render, view, or share the invoice offline or retry upload without data loss.", bullet_style))
    story.append(Paragraph("<b>6. Network Timeout & Retry Mechanics:</b> Streaming multipart uploads operate with a 120-second timeout. If an upload times out mid-transit due to cellular drops, the Order Completion form retains all in-memory file references (captured photo thumbnails and video path). The field operative does NOT need to re-capture any proofs; tapping 'Submit Delivery' immediately re-initiates the multipart payload.", bullet_style))
    story.append(Spacer(1, 4))
